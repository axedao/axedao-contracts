...
describe('IDO', () => {
  ...
  beforeEach(async () => {
    ...
    await uniFactory.createPair(axe.address, dai.address)
    ...
    const bondingCalculator = await BondingCalculator.deploy(axe.address)
    ...
    const IDO = await ethers.getContractFactory('AxeIDO')
    ...
    it('should be blocked by whitelist', async () => {
      await ido.initialize(
        BigNumber.from(200000).mul(
          BigNumber.from(10).pow(await axe.decimals())
        ),
      ...
      await expect(
        ido.purchaseAXE(BigNumber.from(1000).mul(BigNumber.from(10).pow(18)))
  ...
  describe('purchase', () => {
    ...
    it('able to purchase first time', async () => {
      const maxAmountAXE = await ido.getAllotmentPerBuyer()
      const amountDAI = maxAmountAXE.div(1e9).mul(await ido.salePrice())
      await expect(() => ido.purchaseAXE(amountDAI)).to.changeTokenBalance(
        dai,
        deployer,
        amountDAI.mul(-1)
      )
      expect(await ido.totalAmount()).to.eq(
        BigNumber.from(200000).mul(BigNumber.from(10).pow(9)).sub(maxAmountAXE)
      )
      expect(await ido.purchasedAmounts(deployer.address)).to.eq(maxAmountAXE)
      expect(await dai.balanceOf(ido.address)).to.eq(amountDAI)
    })

    it('failed to purchase second time', async () => {
      await ido.purchaseAXE(
        BigNumber.from(1000).mul(BigNumber.from(10).pow(18))
      )
      await expect(
        ido.purchaseAXE(BigNumber.from(1000).mul(BigNumber.from(10).pow(18)))
  ...
  describe('disable white list', () => {
    ...
    it('should able to purchase', async () => {
      ...
      await expect(() =>
        buyerIdo.purchaseAXE(secondAmountDAI)
    ...
    it('should failed to purchase more then total amount', async () => {
      ...
      const buyerIdo = ido.connect(buyer1)
      await expect(
        buyerIdo.purchaseAXE(secondBuyDAIAmount)
  ...
  describe('finalize', () => {
    ...
    it('should finalize', async () => {
      ...
      await ido.connect(buyer1).purchaseAXE(amountDAI)
  ...
  describe.skip('1000 buyer', () => {
    ...
    const pricePerAxe = BigNumber.from(5).mul(BigNumber.from(10).pow(18))
    beforeEach(async () => {})

    it('buy all axe and finalize', async () => {
      const wallets = []
      ...
      await (await ido.purchaseAXE(daiAmount)).wait()
      ...
      for (let i = 0; i < totalBuyer; i += step) {
        await Promise.all(
          wallets.slice(i, i + step).map(async (wallet, j) => {
            ...
            await (await buyerIDO.purchaseAXE(daiAmount)).wait()
  ...
  describe('start', () => {
    ...
    it('should failed to purchase if not initialized', async () => {
      const maxAmountAXE = await ido.getAllotmentPerBuyer()
      const amountDAI = maxAmountAXE.div(1e9).mul(await ido.salePrice())
      await expect(ido.purchaseAXE(amountDAI)).to.be.revertedWith(
        'Not started'
      )
    })
